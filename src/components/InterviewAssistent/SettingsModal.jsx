  ...
  export default function SettingsModal({ onClose }) {
    ...
    if (modelResponse.ok) {
      ...
      let providers = modelData.available_providers || {};

      // 🔥 Merge backend providers with ALL local models
      const mergedProviders = {};
      Object.keys(MODEL_LABELS).forEach(model => {
        mergedProviders[model] =
          providers[model] !== undefined ? providers[model] : true;
      });

      setDefaultModel(modelData.default_provider || 'gpt-4o');
      setCodingModel(modelData.coding_provider || 'gpt-4o');

      // 🔥 Final all-model list with availability flags
      setAvailableProviders(mergedProviders);
    }
  } catch (error) {
    console.warn('Model API not available:', error);

    // 🔥 If backend unreachable → fallback to all models
    const defaultProviders = {};
    Object.keys(MODEL_LABELS).forEach(k => { defaultProviders[k] = true; });
              ...
              {['responseStyle', 'audioTranscription', 'models', 'advancedSettings'].map((tab) => (
                <button
                  ...
                  className={`px-4 py-3 text-sm font-medium transition-colors border-b-2 ${
                    activeTab === tab ? 'border-purple-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'
                  }`}
                >
