...
// import { X, ChevronDown, Check, AlertCircle, Trash2, Edit2, Settings as SettingsIcon } from 'lucide-react';
...
//         });
//       } catch (err) {
//         console.warn('Model API update failed:', err);
//       }
...
//       // Refresh global cache so other components see the new settings immediately
...
//                   activeTab === tab ? 'border-indigo-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'
//                 }`}
//               >
...
//             className="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-600 hover:from-indigo-500 hover:to-indigo-500 disabled:from-gray-700 disabled:to-gray-700 text-white font-semibold transition-all disabled:cursor-not-allowed rounded-lg"
//           >
//             {saving ? 'Saving...' : 'Save'}
//           </button>
//         </div>
...
// /* --------------------------------------------------------------------------
//    Subcomponents (kept logically the same as original; only props wiring used)
//    -------------------------------------------------------------------------- */
...
//             className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white appearance-none focus:outline-none focus:border-indigo-500/50"
//           >
...
//                   <button onClick={() => onEditStyle(selectedStyle)} className="p-2 text-blue-400 hover:text-indigo-300 hover:bg-indigo-500/10 rounded-lg transition-colors" title="Edit style">
...
//             <div>
//               <p className="text-indigo-400 text-sm font-medium mb-2">Example Response</p>
...
//         <button onClick={onCreateStyle} className="mt-6 w-full bg-indigo-600/20 hover:bg-indigo-600/30 border-2 border-indigo-500/50 hover:border-indigo-500 text-indigo-300 font-semibold py-3 px-4 rounded-xl transition-all flex items-center justify-center gap-2">
...
//     try {
//       if (style) {
//         await responseStyleService.updateStyle(style.id, userId, {
...
//         });
//       } else {
//         await responseStyleService.createStyle(userId, {
//           style_name: formData.styleName,
//           description: formData.description,
//           approximate_length: formData.approximateLength,
//           tone: formData.tone,
...
//               <input type="text" value={formData.styleName} onChange={(e) => setFormData({...formData, styleName: e.target.value})} placeholder="e.g., Brief Professional, Storyteller, Technical Expert" className="w-full bg-gray-800 text-white px-4 py-3 rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
//             </div>

//             <div>
//               <label className="text-white font-medium mb-2 block">Description <span className="text-red-400">*</span></label>
//               <input type="text" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} placeholder="Brief description of this response style" className="w-full bg-gray-800 text-white px-4 py-3 rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
//             </div>

//             <div className="grid grid-cols-2 gap-4">
//               <div>
//                 <label className="text-white font-medium mb-2 block">Approximate Length</label>
//                 <input type="text" value={formData.approximateLength} onChange={(e) => setFormData({...formData, approximateLength: e.target.value})} placeholder="e.g., ~150 words" className="w-full bg-gray-800 text-white px-4 py-3 rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
//               </div>

//               <div>
//                 <label className="text-white font-medium mb-2 block">Tone</label>
//                 <select value={formData.tone} onChange={(e) => setFormData({...formData, tone: e.target.value})} className="w-full bg-gray-800 text-white px-4 py-3 rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
...
//               <textarea value={formData.exampleResponse} onChange={(e) => setFormData({...formData, exampleResponse: e.target.value})} placeholder="Write a sample response that demonstrates this style..." className="w-full bg-gray-800 text-white px-4 py-3 rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[200px] resize-none" />
...
//             <button type="submit" disabled={saving} className="flex-1 px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 disabled:from-gray-700 disabled:to-gray-700 text-white font-semibold rounded-lg transition-all disabled:cursor-not-allowed">
...
// function AdvancedSettingsTab({ messageDirection, setMessageDirection, autoScroll, setAutoScroll, programmingLanguage, setProgrammingLanguage, interviewInstructions, setInterviewInstructions, codingInstructions, setCodingInstructions }) {
//   return (
//     <div className="space-y-6">
//       <div>
//         <div className="flex items-center justify-between mb-3">
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-gray-200 shadow-2xl">
        <div className="bg-gradient-to-r from-gray-50 to-white px-6 py-4 flex items-center justify-between border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">Copilot Parameters</h2>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          {loading || globalLoading ? (
            <div className="text-center text-gray-600 py-8">Loading settings...</div>
          ) : (
            <>
              {activeTab === 'responseStyle' && (
                <ResponseStyleTab
        ...
        <div className="bg-gray-50 px-6 py-4 flex items-center justify-end gap-3 border-t border-gray-200">
          <button onClick={onClose} className="px-6 py-2.5 bg-white hover:bg-gray-100 text-gray-700 border border-gray-200 rounded-lg font-medium transition-colors shadow-sm">Close</button>
          <button
            onClick={handleSave}
            disabled={saving || loading || globalLoading}
            className="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 disabled:from-gray-300 disabled:to-gray-400 text-white font-semibold transition-all disabled:cursor-not-allowed rounded-lg shadow-md hover:shadow-lg"
          >
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
...
function ResponseStyleTab({ responseStyles, selectedStyleId, setSelectedStyleId, onCreateStyle, onEditStyle, onDeleteStyle }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <label className="text-gray-900 font-semibold mb-2 block">Copilot Style</label>
        <p className="text-gray-600 text-sm mb-4">Select how your AI assistant responds</p>

        <div className="relative mb-6">
          <select
            value={selectedStyleId || ''}
            onChange={(e) => setSelectedStyleId(e.target.value)}
            className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-gray-900 appearance-none focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 shadow-sm"
          >
            {responseStyles.length === 0 && <option value="">No styles available</option>}
            {responseStyles.map(style => (
              <option key={style.id} value={style.id}>
        ...
        {selectedStyle && (
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-200 shadow-sm">
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <h3 className="text-gray-900 font-semibold text-lg mb-2">{selectedStyle.style_name}</h3>
                <p className="text-gray-600 text-sm mb-4">{selectedStyle.description}</p>

                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <p className="text-gray-500 text-xs mb-1">Approximate Length</p>
                    <p className="text-gray-700 text-sm">{selectedStyle.approximate_length}</p>
                  </div>
                  <div>
                    <p className="text-gray-500 text-xs mb-1">Tone</p>
                    <p className="text-gray-700 text-sm">{selectedStyle.tone}</p>
                  </div>
                </div>
              </div>
              ...
              {!selectedStyle.is_system_default && (
                <div className="flex gap-2 ml-4">
                  <button onClick={() => onEditStyle(selectedStyle)} className="p-2 text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit style">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => onDeleteStyle(selectedStyle.id)} className="p-2 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors" title="Delete style">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              )}
            ...
            <div>
              <p className="text-indigo-600 text-sm font-medium mb-2">Example Response</p>
              <div className="bg-white border border-gray-200 rounded-lg p-4 max-h-64 overflow-y-auto shadow-sm">
                <p className="text-gray-700 text-sm whitespace-pre-wrap leading-relaxed">{selectedStyle.example_response}</p>
              </div>
            </div>
          </div>
        )}

        <button onClick={onCreateStyle} className="mt-6 w-full bg-indigo-50 hover:bg-indigo-100 border-2 border-indigo-200 hover:border-indigo-300 text-indigo-700 font-semibold py-3 px-4 rounded-xl transition-all flex items-center justify-center gap-2 shadow-sm">
          <SettingsIcon className="w-5 h-5" />
          Style Customization
        </button>
      </div>
...
function StyleCustomizationModal({ style, onClose, onSave, userId }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden border border-gray-200 shadow-2xl">
        <div className="bg-gradient-to-r from-gray-50 to-white px-6 py-4 flex items-center justify-between border-b border-gray-200">
          <h3 className="text-xl font-bold text-gray-900">{style ? 'Edit Response Style' : 'Create Custom Style'}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          <div className="space-y-5">
            <div>
              <label className="text-gray-900 font-medium mb-2 block">Style Name <span className="text-red-500">*</span></label>
              <input type="text" value={formData.styleName} onChange={(e) => setFormData({...formData, styleName: e.target.value})} placeholder="e.g., Brief Professional, Storyteller, Technical Expert" className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 shadow-sm" />
            </div>

            <div>
              <label className="text-gray-900 font-medium mb-2 block">Description <span className="text-red-500">*</span></label>
              <input type="text" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} placeholder="Brief description of this response style" className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 shadow-sm" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-gray-900 font-medium mb-2 block">Approximate Length</label>
                <input type="text" value={formData.approximateLength} onChange={(e) => setFormData({...formData, approximateLength: e.target.value})} placeholder="e.g., ~150 words" className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 shadow-sm" />
              </div>

              <div>
                <label className="text-gray-900 font-medium mb-2 block">Tone</label>
                <select value={formData.tone} onChange={(e) => setFormData({...formData, tone: e.target.value})} className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 shadow-sm">
                  <option>Professional</option>
                  <option>Casual</option>
                  <option>Friendly</option>
                  <option>Formal</option>
            ...
            <div>
              <label className="text-gray-900 font-medium mb-2 block">Example Response <span className="text-red-500">*</span></label>
              <p className="text-gray-600 text-sm mb-2">Provide an example answer in this style to help the AI understand your preferences</p>
              <textarea value={formData.exampleResponse} onChange={(e) => setFormData({...formData, exampleResponse: e.target.value})} placeholder="Write a sample response that demonstrates this style..." className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 min-h-[200px] resize-none shadow-sm" />
              <p className="text-gray-500 text-xs mt-1">{formData.exampleResponse.length} characters</p>
            </div>
          ...
          <div className="flex gap-3 mt-6 pt-6 border-t border-gray-200">
            <button type="button" onClick={onClose} className="flex-1 px-6 py-3 bg-white hover:bg-gray-100 text-gray-700 border border-gray-200 rounded-lg font-medium transition-colors shadow-sm">Cancel</button>
            <button type="submit" disabled={saving} className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 disabled:from-gray-300 disabled:to-gray-400 text-white font-semibold rounded-lg transition-all disabled:cursor-not-allowed shadow-md hover:shadow-lg">
              {saving ? 'Saving...' : style ? 'Update Style' : 'Create Style'}
            </button>
          </div>
        </form>
...
function AudioTranscriptionTab({ audioLanguage, setAudioLanguage, pauseInterval, setPauseInterval, advancedQuestionDetection, setAdvancedQuestionDetection }) {
  return (
    <div className="space-y-6">
      <div>
        <label className="text-gray-900 font-medium mb-2 block">Language</label>
        <p className="text-gray-600 text-sm mb-3">Language of the interview</p>
        <select value={audioLanguage} onChange={(e) => setAudioLanguage(e.target.value)} className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 shadow-sm">
          <option>English</option>
          <option>Spanish</option>
          <option>French</option>
          <option>German</option>
      ...
      <div>
        <label className="text-gray-900 font-medium mb-2 block">Pause Interval</label>
        <p className="text-gray-600 text-sm mb-3">Duration of pause to detect the end of speech and generate a new transcription paragraph (seconds)</p>
        <div className="flex items-center gap-4">
          <input type="number" min="1" max="10" value={pauseInterval} onChange={(e) => setPauseInterval(Number(e.target.value))} className="flex-1 bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 shadow-sm" />
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            <label className="text-gray-900 font-medium block">Advanced Question Detection</label>
            <span className="bg-indigo-100 text-indigo-700 text-xs px-2 py-1 rounded-md inline-block mt-1">Experimental</span>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" checked={advancedQuestionDetection} onChange={(e) => setAdvancedQuestionDetection(e.target.checked)} className="sr-only peer" />
        ...
        <p className="text-gray-600 text-sm">Intelligently filter conversation to focus on meaningful questions</p>
      </div>
    </div>
  );
}
...
function ModelsTab({ defaultModel, setDefaultModel, codingModel, setCodingModel, modelOptions }) {
  return (
    <div className="space-y-6">
      <div>
        <label className="text-gray-900 font-semibold mb-2 block">Default Copilot Model</label>
        <p className="text-gray-600 text-sm mb-4">Select the AI model for general copilot interactions. Automatic fallback enabled if primary model fails.</p>
        <div className="relative">
          <select value={defaultModel} onChange={(e) => setDefaultModel(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-gray-900 appearance-none focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 shadow-sm">
            {modelOptions.map(option => (
              <option key={option.value} value={option.value} disabled={!option.available}>
                {option.label} {!option.available ? '(Not Available)' : ''}
              </option>
      ...
      <div>
        <label className="text-gray-900 font-semibold mb-2 block">Coding Assistant Model</label>
        <p className="text-gray-600 text-sm mb-4">Select the AI model for coding-related tasks and technical questions</p>
        <div className="relative">
          <select value={codingModel} onChange={(e) => setCodingModel(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-gray-900 appearance-none focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 shadow-sm">
            {modelOptions.map(option => (
              <option key={option.value} value={option.value} disabled={!option.available}>
                {option.label} {!option.available ? '(Not Available)' : ''}
              </option>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-blue-700 text-sm font-medium mb-1">Automatic Fallback Enabled</p>
            <p className="text-blue-600 text-xs">If your primary model fails or is unavailable, the system will automatically switch to the backup model to ensure uninterrupted service.</p>
          </div>
        </div>
      </div>
    </div>
...
function AdvancedSettingsTab({ messageDirection, setMessageDirection, autoScroll, setAutoScroll, programmingLanguage, setProgrammingLanguage, interviewInstructions, setInterviewInstructions, codingInstructions, setCodingInstructions }) {
  return (
    <div className="space-y-6">
      <div>
        <div className="flex items-center justify-between mb-3">
          <label className="text-gray-900 font-medium">Message Direction</label>
          <label className="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" checked={messageDirection === 'bottom'} onChange={(e) => setMessageDirection(e.target.checked ? 'bottom' : 'top')} className="sr-only peer" />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-100 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
          </label>
        </div>
        <p className="text-gray-600 text-sm">Newer Interview Copilot messages appear at the {messageDirection}</p>
      </div>

      <div>
        <div className="flex items-center justify-between mb-3">
          <label className="text-gray-900 font-medium">Auto Scroll</label>
          <label className="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" checked={autoScroll} onChange={(e) => setAutoScroll(e.target.checked)} className="sr-only peer" />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-100 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
          </label>
        </div>
        <p className="text-gray-600 text-sm">Enable auto scroll by default</p>
      </div>
      <div>
        <label className="text-gray-900 font-medium mb-2 block">Programming Language</label>
        <p className="text-gray-600 text-sm mb-3">Default Programming Language</p>
        <select value={programmingLanguage} onChange={(e) => setProgrammingLanguage(e.target.value)} className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 shadow-sm">
          <option>Python</option>
          <option>JavaScript</option>
          <option>Java</option>
          <option>C++</option>
      ...
      <div>
        <label className="text-gray-900 font-medium mb-2 block">Additional Instruction (Interview Copilot)</label>
        <textarea value={interviewInstructions} onChange={(e) => setInterviewInstructions(e.target.value)} placeholder="Tell AI what to do or not to do (e.g. 'Don't use jargon' or 'Use most recent work experience')" className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 min-h-[100px] resize-none shadow-sm" />
      </div>

      <div>
        <label className="text-gray-900 font-medium mb-2 block">Additional Instruction (Coding Copilot)</label>
        <textarea value={codingInstructions} onChange={(e) => setCodingInstructions(e.target.value)} placeholder="Instructions for coding solutions (e.g. 'Prefer clear code over optimized code' or 'Include detailed comments')" className="w-full bg-white text-gray-900 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 min-h-[100px] resize-none shadow-sm" />
      </div>
    </div>
  );
}