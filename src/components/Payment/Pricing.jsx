...
export default function Pricing() {
  ...
  return (
    <>
      ...
      <section
        id="pricing"
        className="min-h-screen py-16 bg-gradient-to-br from-gray-50 via-purple-50 to-indigo-50"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
            ...
            {modernPlans.map((plan, idx) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  <span
                    className={`${plan.badgeColor} px-3 sm:px-4 py-1 rounded-full text-xs font-bold`}
                  >
                    {plan.badge}
                  </span>
                ...
                <h3
                  className={`text-base sm:text-lg mb-4 ${
                    plan.cardStyle.includes("bg-indigo-600")
                      ? "text-white"
                      : "text-gray-600"
                  }`}
                >
                ...
                <div className="mb-6">
                  {plan.originalPrice && (
                    <span
                      className={`text-xl sm:text-2xl line-through mr-2 ${
                        plan.cardStyle.includes("bg-indigo-600")
                          ? "text-white/70"
                          : "text-gray-400"
                      }`}
                ...
                {/* CTA Button - placeholder for payment gateway */}
                <button
                  onClick={() =>
                    console.log("Payment flow coming soon")
                  }
                  className={`w-full py-3 rounded-xl font-semibold transition-all text-sm sm:text-base ${plan.buttonStyle}`}
                >
                  {plan.buttonText} →
                </button>
              </div>
          ...
          <div className="bg-gray-900 rounded-3xl p-8 sm:p-12 text-center text-white">
            ...
            {/* <-- Restored navigation for this banner button as requested */}
            <button
              onClick={() => navigate("/interview")}
              className="bg-yellow-400 text-gray-900 px-8 py-3 rounded-xl font-bold hover:bg-yellow-300 transition-all"
            >
              Start Free Trial
            </button>
          </div>
