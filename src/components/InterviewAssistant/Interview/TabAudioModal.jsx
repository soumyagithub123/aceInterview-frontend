...
//           <button
//             onClick={() => {
//               console.log("🖱️ Select Tab Audio clicked");
//               onSelect();
//             }}
...
export default function TabAudioModal({ onSelect, onClose, errorMessage }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-3 sm:px-4 py-4">
      ...
      <div className="relative w-full max-w-xl max-h-[92vh] sm:max-h-[90vh] overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 bg-[#0b0b10] shadow-[0_40px_140px_-60px_rgba(0,0,0,0.85)]">
        
        {/* Top glow */}
        <div className="pointer-events-none absolute -top-16 sm:-top-24 left-1/2 h-32 sm:h-40 lg:h-48 w-80 sm:w-96 lg:w-[36rem] -translate-x-1/2 rounded-full bg-gradient-to-r from-purple-500/30 via-fuchsia-500/20 to-cyan-500/20 blur-3xl" />

        {/* Header - Responsive */}
        <div className="relative flex flex-wrap items-start justify-between gap-2 sm:gap-3 px-4 sm:px-5 lg:px-6 pt-4 sm:pt-5 lg:pt-6 pb-3 sm:pb-4 border-b border-white/10">
          
          <div className="flex items-start gap-2 sm:gap-3 min-w-0 flex-1">
            <div className="relative shrink-0">
              <div className="absolute -inset-1.5 sm:-inset-2 rounded-xl sm:rounded-2xl bg-emerald-500/15 blur-xl" />
              <div className="relative h-9 w-9 sm:h-10 sm:w-10 lg:h-11 lg:w-11 rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 flex items-center justify-center">
                <Volume2 className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-200" />
              </div>
            </div>

            <div className="min-w-0 flex-1">
              <h2 className="text-sm sm:text-base lg:text-xl font-semibold text-white tracking-tight">
                Enable Interviewer Audio
              </h2>
              <p className="mt-1 text-[11px] sm:text-xs lg:text-sm text-zinc-400 leading-relaxed">
                Capture the interviewer's voice directly from your meeting tab.
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="h-8 w-8 sm:h-9 sm:w-9 lg:h-10 lg:w-10 rounded-lg sm:rounded-xl border border-white/10 bg-white/5 text-zinc-300 hover:text-white hover:bg-white/10 transition flex items-center justify-center shrink-0"
            aria-label="Close"
            title="Close"
          >
        ...
        {/* Body - Responsive Scrolling */}
        <div className="relative px-4 sm:px-5 lg:px-6 py-3 sm:py-4 lg:py-5 space-y-3 sm:space-y-4 overflow-y-auto max-h-[calc(92vh-200px)] sm:max-h-[calc(90vh-180px)]">
          
          {/* Steps - Responsive */}
          <div className="rounded-xl sm:rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.03] p-3 sm:p-4 lg:p-5">
            
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-purple-200 shrink-0" />
                <p className="text-[11px] sm:text-xs lg:text-sm font-semibold text-zinc-100">
                  Quick setup (30 seconds)
                </p>
              </div>
              <span className="text-[9px] sm:text-[10px] lg:text-xs text-zinc-500 whitespace-nowrap">
                Zoom • Meet • Teams
              </span>
            </div>
            ...
            <div className="mt-3 sm:mt-4 grid gap-2 sm:gap-3">
              {[
                ...
                <div
                  key={i}
                  className="flex items-start gap-2 sm:gap-3 rounded-lg sm:rounded-xl border border-white/10 bg-black/20 p-2.5 sm:p-3"
                >
                  <div className="mt-0.5 h-6 w-6 sm:h-7 sm:w-7 shrink-0 rounded-lg sm:rounded-xl bg-purple-500/15 border border-purple-500/20 flex items-center justify-center">
                    <span className="text-[10px] sm:text-xs font-bold text-purple-200">
                      {i + 1}
                    </span>
                  </div>
                  <p className="text-[11px] sm:text-xs lg:text-sm text-zinc-200 leading-relaxed break-words flex-1">
                    {t}
                  </p>
                </div>
              ))}
          ...
          {/* Error - Responsive */}
          {errorMessage && (
            <div className="rounded-xl sm:rounded-2xl border border-rose-500/25 bg-rose-500/10 p-3 sm:p-4 flex items-start gap-2 sm:gap-3">
              <div className="h-8 w-8 sm:h-9 sm:w-9 lg:h-10 lg:w-10 rounded-xl sm:rounded-2xl border border-rose-500/25 bg-rose-500/10 flex items-center justify-center shrink-0">
                <AlertCircle className="h-4 w-4 sm:h-5 sm:w-5 text-rose-200" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[11px] sm:text-xs lg:text-sm font-semibold text-rose-100">
                  Error
                </p>
                <p className="mt-1 text-[11px] sm:text-xs lg:text-sm text-rose-200/90 break-words">
                  {errorMessage}
                </p>
              </div>
            </div>
          ...
          <button
            ...
            className="group relative w-full overflow-hidden rounded-xl sm:rounded-2xl border border-emerald-300/15 bg-gradient-to-b from-emerald-400 to-emerald-500 text-black font-semibold py-2.5 sm:py-3 lg:py-3.5 px-4 sm:px-5 lg:px-6 shadow-[0_20px_60px_-35px_rgba(16,185,129,0.55)] transition hover:brightness-[1.02] active:brightness-[0.98]"
          >
            <span className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity bg-gradient-to-r from-white/0 via-white/20 to-white/0" />
            <span className="relative inline-flex items-center justify-center gap-2 text-xs sm:text-sm lg:text-base">
          ...
          {/* Info - Responsive */}
          <div className="rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 p-3 sm:p-4 flex items-start gap-2 sm:gap-3">
            <div className="h-7 w-7 sm:h-8 sm:w-8 lg:h-9 lg:w-9 rounded-xl sm:rounded-2xl border border-white/10 bg-black/20 flex items-center justify-center shrink-0">
              <Info className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-zinc-200" />
            </div>
            <p className="text-[11px] sm:text-xs lg:text-sm text-zinc-400 leading-relaxed break-words flex-1">
              Your microphone is already active. This step captures the
              interviewer's voice from your browser tab audio.
            </p>
          </div>
        ...
        <div className="relative px-4 sm:px-5 lg:px-6 pb-3 sm:pb-4 lg:pb-6">
          <p className="text-[10px] sm:text-[11px] text-zinc-500 break-words leading-relaxed">
            Tip: If you don't see "Share tab audio", try selecting a different
            tab or use Chrome-based browsers.
          </p>
        </div>
